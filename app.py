"""
app.py — Dashboard Gemelo Digital · Dora del Hoyo
"""
...
from datos import (
    ...
    CAPACIDAD_BASE, TAMANO_LOTE_BASE,
)
...
def kpi_card(label, value, unit="", color="#6366F1", icon="▸"):
    ...
    ], style={**CARD_SM,"minWidth":"130px","borderTop":f"2px solid {color}"})
 
def page_header(title, subtitle=""):
    return html.Div([
        html.Div(subtitle, style={
            "fontSize":"9px","letterSpacing":"0.22em","color":"#6366F1",
            "fontFamily":"'DM Mono',monospace","textTransform":"uppercase","marginBottom":"4px",
        }),
        html.H2(title, style={
...
def section_label(text):
    return html.Div([
        ...
        html.Div(style={"height":"1px","flex":"1","marginLeft":"12px",
...
def tabla_dash(df, id_tabla, page_size=12):
    if df is None or (hasattr(df,"empty") and df.empty):
        return html.Div("Sin datos", style={"color":"#475569"})
    df_r = df.copy()
    ...
    return dash_table.DataTable(
        ...
        style_header={
            ...
            "letterSpacing":"0.1em","textTransform":"uppercase","padding":"10px 12px",
        },
        style_cell={
...
def no_data_msg():
    return html.Div([
        ...
        html.Div("Ejecuta el pipeline primero", style={
            "textAlign":"center","color":"#475569","fontSize":"14px",
            "fontFamily":"'Outfit',sans-serif",
        }),
...
sidebar = html.Div([
    html.Div([
        html.Div("◈", style={"fontSize":"32px","color":"#6366F1",
                              "fontFamily":"'Outfit',sans-serif","lineHeight":"1"}),
        html.Div("DORA", style={"fontSize":"18px","fontWeight":"800","color":"#F1F5F9",
                                 "fontFamily":"'Outfit',sans-serif","letterSpacing":"0.08em",
                                 "lineHeight":"1","marginTop":"4px"}),
        html.Div("DEL HOYO", style={"fontSize":"18px","fontWeight":"800","color":"#F1F5F9",
                                     "fontFamily":"'Outfit',sans-serif","letterSpacing":"0.08em",
                                     "lineHeight":"1"}),
        html.Div("GEMELO DIGITAL", style={"fontSize":"8px","color":"#6366F1",
                                           "letterSpacing":"0.2em",
                                           "fontFamily":"'DM Mono',monospace","marginTop":"6px"}),
    ], style={"padding":"28px 20px 22px","borderBottom":"1px solid rgba(148,163,184,0.1)",
              "marginBottom":"8px"}),
    html.Div([
        html.Button(
            [html.Span(icon+" ", style={"fontSize":"14px","marginRight":"4px"}),
             html.Div([
                 html.Span(num, style={"fontSize":"8px","color":"#475569",
                                       "fontFamily":"'DM Mono',monospace",
                                       "display":"block","lineHeight":"1"}),
                 html.Span(label, style={"fontSize":"12px","lineHeight":"1.2"}),
             ])],
            id=f"btn-{tab}", n_clicks=0,
    ...
    html.Div([
        html.Div("v2.0 · 2025", style={"fontSize":"9px","color":"#334155",
                                        "fontFamily":"'DM Mono',monospace"}),
...
<head>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box}
        ...
        body{background:radial-gradient(ellipse at 20% 50%,rgba(99,102,241,0.05) 0%,transparent 50%),radial-gradient(ellipse at 80% 20%,rgba(14,165,233,0.04) 0%,transparent 50%),#080E1C}
        /* Dropdown */
        ...
        /* Sliders */
        .rc-slider-track{background:linear-gradient(90deg,#6366F1,#0EA5E9)!important}
        .rc-slider-handle{border-color:#6366F1!important;background:#6366F1!important}
        .rc-slider-tooltip-inner{background:#1E293B!important;color:#E2E8F0!important;font-family:'DM Mono',monospace!important;font-size:10px!important}
...
# ─── LAYOUT — los controles SIEMPRE están en el DOM ─────────────────────────
...
app.layout = html.Div([
    ...
    dcc.Interval(id="auto-run", interval=1200, max_intervals=1),
    ...
    html.Div([
        # Header
        html.Div([
            html.Div(id="header-breadcrumb", children="PLANTA DE PRODUCCIÓN  ›  MÓDULO 01",
                     style={"fontSize":"9px","letterSpacing":"0.2em","color":"#334155",
                            "fontFamily":"'DM Mono',monospace"}),
            html.Div(id="header-title", children="DEMANDA",
                     style={"fontFamily":"'Outfit',sans-serif","fontWeight":"800",
                            "fontSize":"13px","color":"#64748B","letterSpacing":"0.12em",
                            "marginLeft":"24px"}),
        ], style={"padding":"14px 32px 12px","borderBottom":"1px solid rgba(148,163,184,0.07)",
                   "background":"rgba(15,23,42,0.6)","display":"flex","alignItems":"center",
                   "position":"sticky","top":"0","zIndex":"100"}),
 
        html.Div([
 
            # ── Panel de parámetros — SIEMPRE en DOM, se muestra/oculta con display ──
            html.Div(id="params-panel", children=[
                html.Div([
                    dbc.Row([
                        dbc.Col([
                            ...
                            dcc.Dropdown(
                                ...
                                style={"minWidth":"155px","background":"rgba(15,23,42,0.9)",
                                       "color":"#E2E8F0"},
                                className="dd-dark",
                            ),
                        ], width="auto"),
                        ...
                        dbc.Col([
                            ...
                            dbc.Checklist(
                                ...
                                options=[{"label":" Falla en horno","value":"falla"},
                                         {"label":" Doble turno","value":"turno"}],
                                value=[], switch=True,
                            ),
                        ], width="auto"),
                        dbc.Col([
                            html.Div(style={"height":"20px"}),
                            html.Button("▶  EJECUTAR", id="btn-run", n_clicks=0,
                    ...
                    html.Div(id="run-status", style={"marginTop":"10px","fontSize":"11px",
                                                      "color":"#10B981",
                                                      "fontFamily":"'DM Mono',monospace"}),
                ], style=CARD),
                html.Div(style={"height":"20px"}),
            ...
            dcc.Loading(id="loading-main", type="dot", color="#6366F1",
                children=html.Div(id="tab-content")),
 
        ], style={"padding":"24px 32px 40px"}),
 
    ], style={"marginLeft":"200px","minHeight":"100vh","background":"transparent"}),
...
def cambiar_tab(*args):
    ...
    return tab_id, f"PLANTA DE PRODUCCIÓN  ›  MÓDULO {num}", label.upper()
 
 
# ── Mostrar/ocultar panel de parámetros ──────────────────────────────────────
 
@app.callback(
    Output("params-panel", "style"),
    Input("store-active-tab", "data"),
)
def toggle_params(tab):
    # Demanda no necesita parámetros
    if tab == "tab-demanda":
        return {"display": "none"}
...
def ejecutar_pipeline(n_clicks, _auto, mes_idx, factor_dem, cap_horno, opciones):
    try:
        fd = float(factor_dem or 1.0)
 
        dem_h = calcular_dem_horas(factor=fd)
        df_agr, costo = run_agregacion(dem_h)
        prod_hh = dict(zip(df_agr["Mes"], df_agr["Produccion_HH"]))
 
        desag = run_desagregacion(prod_hh, fd)
        ...
        status = (f"✓  Pipeline listo  ·  {len(df_l)} lotes  ·  mes: {mes_nm}  ·  "
                  f"COP ${costo:,.0f}")
        return (
            df_agr.to_json(),
        ...
        return (None,)*7 + (f"✗ Error: {str(e)} | {tb[-300:]}",)
...
def render_tab(tab, agr_j, desag_j, sim_j, util_j, kpi_j, sen_j,
    ...
    if tab == "tab-demanda":
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col(html.Div(dcc.Graph(figure=fig_barras_demanda(THEME),
                                           config={"displayModeBar":False}), style=CARD), width=8),
                dbc.Col(html.Div(dcc.Graph(figure=fig_heatmap_demanda(THEME),
                                           config={"displayModeBar":False}), style=CARD), width=4),
            ], className="g-3 mb-4"),
    ...
    if tab == "tab-agregacion":
        ...
        return html.Div([
            ...
            html.Div(dcc.Graph(figure=fig_plan_agregado(df_agr, costo, THEME),
    ...
    if tab == "tab-desag":
        ...
        return html.Div([
            ...
            dbc.Row([
                ...
                *[dbc.Col(kpi_card(p.replace("_"," "),
                                   f"{desag_dict[p]['Produccion'].sum():,.0f}",
                                   "und", PROD_COLORS[p]), width="auto")
                  for p in PRODUCTOS],
            ], className="g-3 mb-4"),
            html.Div(dcc.Graph(figure=fig_desagregacion(desag_dict, mes_nm, THEME),
                               config={"displayModeBar":False}), style=CARD),
            section_label("tablas por producto"),
            dbc.Row([
                dbc.Col(html.Div([
                    html.Div(p.replace("_"," "), style={"fontSize":"10px","color":PROD_COLORS[p],
                                                         "fontFamily":"'Outfit',sans-serif",
    ...
    if tab == "tab-sim":
        ...
        return html.Div([
            page_header("Simulación de Eventos Discretos", "módulo 04 · SimPy · flujo de lotes"),
            dbc.Row([
                dbc.Col(kpi_card("Lotes Simulados",    len(df_l),                              "",    "#6366F1"), width="auto"),
                dbc.Col(kpi_card("Tiempo Total",        f"{df_l['t_fin'].max():,.0f}",         "min", "#0EA5E9"), width="auto"),
                dbc.Col(kpi_card("Duración Prom Lote",  f"{df_l['tiempo_sistema'].mean():.1f}","min", "#10B981"), width="auto"),
                dbc.Col(kpi_card("Productos Activos",   len(df_l["producto"].unique()),         "",   "#F59E0B"), width="auto"),
            ], className="g-3 mb-4"),
            html.Div(dcc.Graph(figure=fig_gantt(df_l, THEME, n=80),
                               config={"displayModeBar":False}), style=CARD),
            html.Div(style={"height":"16px"}),
            html.Div(dcc.Graph(figure=fig_colas(df_u_r, THEME),
                               config={"displayModeBar":False}), style=CARD),
            section_label("registro de lotes simulados"),
            html.Div(tabla_dash(
                df_l.head(200)[["lote_id","producto","tamano","t_creacion","t_fin",
                                "tiempo_sistema","total_espera"]],
                "tbl-lotes"), style=CARD),
        ])
    ...
    if tab == "tab-kpis":
        ...
        return html.Div([
            page_header("KPIs & Cuellos de Botella", "módulo 05 · métricas de desempeño"),
            dbc.Row([
                dbc.Col(kpi_card("Cumplimiento",   f"{avg_cumpl}", "%",     "#10B981"), width="auto"),
                dbc.Col(kpi_card("Throughput",      f"{avg_tp}",   "und/h", "#6366F1"), width="auto"),
                dbc.Col(kpi_card("Util. Máxima",    f"{max_util}", "%",     "#F59E0B"), width="auto"),
                dbc.Col(kpi_card("Cuellos Botella", cuellos,        "",     "#EF4444"), width="auto"),
            ], className="g-3 mb-4"),
            dbc.Row([
                dbc.Col(html.Div(dcc.Graph(figure=fig_kpis_radar(df_kpi, THEME),
                                           config={"displayModeBar":False}), style=CARD), width=6),
                dbc.Col(html.Div(dcc.Graph(figure=fig_utilizacion(df_u_r, THEME),
                                           config={"displayModeBar":False}), style=CARD), width=6),
            ], className="g-3 mb-4"),
            section_label("tabla KPIs por producto"),
            html.Div(tabla_dash(df_kpi, "tbl-kpis", page_size=10), style=CARD),
            section_label("utilización de recursos"),
            html.Div(tabla_dash(df_ut, "tbl-util", page_size=10) if not df_ut.empty else html.Div(), style=CARD),
        ])
    ...
    if tab == "tab-sensores":
        ...
        return html.Div([
            page_header("Sensores Virtuales", "módulo 06 · monitoreo del horno"),
            dbc.Row([
                dbc.Col(kpi_card("Temp. Máxima",   f"{df_s['temperatura'].max():.1f}",  "°C", "#EF4444"), width="auto"),
                dbc.Col(kpi_card("Temp. Mínima",   f"{df_s['temperatura'].min():.1f}",  "°C", "#0EA5E9"), width="auto"),
                dbc.Col(kpi_card("Temp. Promedio", f"{df_s['temperatura'].mean():.1f}", "°C", "#F59E0B"), width="auto"),
                dbc.Col(kpi_card("Lecturas",        len(df_s),                           "",   "#10B981"), width="auto"),
                dbc.Col(kpi_card("Alertas >200°C",  alertas,                             "",   "#EC4899"), width="auto"),
            ], className="g-3 mb-4"),
            html.Div(dcc.Graph(figure=fig_sensores(df_s, THEME),
                               config={"displayModeBar":False}), style=CARD),
            section_label("registro de lecturas"),
            html.Div(tabla_dash(df_s.round(2), "tbl-sensores", page_size=12), style=CARD),
        ])
 
    # ── 07 ESCENARIOS ─────────────────────────────────────────────────────────
    if tab == "tab-escenarios":
        fig_comp = fig_comparacion_escenarios(esc_store, THEME) if esc_store else go.Figure()
        return html.Div([
            page_header("Análisis de Escenarios What-If", "módulo 07 · evaluación comparativa"),
            html.Div([
                dbc.Row([
                    dbc.Col([
                        html.Span("SELECCIONAR ESCENARIOS", style=LABEL),
                        dcc.Checklist(
                            id="dd-esc", options=ESC_OPTIONS,
            ...
            html.Div(style={"height":"16px"}),
            html.Div(dcc.Graph(id="fig-comp", figure=fig_comp,
                               config={"displayModeBar":False}), style=CARD),
            section_label("descripción de escenarios"),
            dbc.Row([
                dbc.Col(html.Div([
                    html.Div(cfg["label"], style={"color":"#A5B4FC","fontSize":"11px",
                                                    "fontFamily":"'Outfit',sans-serif",
                                                    "fontWeight":"700","marginBottom":"4px"}),
                    html.Div(f"fd={cfg['fd']}  falla={cfg['falla']}  ft={cfg['ft']}  dh={cfg['dh']}",
