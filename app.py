...
# ─────────────────────────────────────────────────────────────────────────────
# PANELES DE CONFIGURACIÓN CONDICIONALES POR TAB
#
# · tab-demanda      → sin parámetros (solo botón ejecutar)
# · tab-agregacion   → sin parámetros (solo botón ejecutar)
# · tab-desag        → selector de mes (para resaltar estrella) + botón
# · tab-sim          → todos los parámetros
# · tab-kpis         → todos los parámetros (depende de simulación)
# · tab-sensores     → todos los parámetros (depende de simulación)
# · tab-escenarios   → sin panel (tiene su propio panel interno)
# ─────────────────────────────────────────────────────────────────────────────

# Tabs que usan parámetros de simulación
TABS_SIM   = {"tab-sim", "tab-kpis", "tab-sensores"}
# Tab que solo necesita selector de mes
TABS_MES   = {"tab-desag"}
# Tabs sin ningún parámetro
TABS_NOPAR = {"tab-demanda", "tab-agregacion", "tab-escenarios"}

def _btn_ejecutar(extra_cols=None):
    """Fila con botón ejecutar + status. extra_cols: lista de dbc.Col adicionales."""
    cols = []
    if extra_cols:
    ...
    cols.append(
        dbc.Col(
            html.Button(
                "▶  EJECUTAR PIPELINE", id="btn-run", n_clicks=0,
                style={
                    "background":"#E8A838","color":"#0a0d11","border":"none",
    ...
    cols.append(
        dbc.Col(
            html.Div(id="run-status", style={
                "color":"#81C784","fontSize":"12px",
                "fontFamily":"IBM Plex Mono, monospace","padding":"10px 0",
            }), width="auto"
        )
    )
    return dbc.Row(cols)
...
def build_panel_config(tab):
    """
    Devuelve el contenido interno del panel de configuración
    según el tab activo.
    """

    # ── DEMANDA y AGREGACIÓN: sin parámetros ──────────────────────────────────
    if tab in TABS_NOPAR:
        return html.Div([
            seccion_titulo("CONFIGURACIÓN", "sin parámetros adicionales para esta sección"),
            _btn_ejecutar(),
        ])

    # ── DESAGREGACIÓN: solo selector de mes ───────────────────────────────────
    if tab in TABS_MES:
        col_mes = dbc.Col([
            html.Div("MES A VISUALIZAR", style=LABEL_STYLE),
            dcc.Dropdown(
                id="dd-mes",
        ...
        return html.Div([
            seccion_titulo("CONFIGURACIÓN", "selecciona el mes a destacar en la desagregación"),
            _btn_ejecutar(extra_cols=[col_mes]),
        ])

    # ── SIMULACIÓN / KPIs / SENSORES: parámetros completos ───────────────────
    if tab in TABS_SIM:
        return html.Div([
            seccion_titulo("CONFIGURACIÓN DE SIMULACIÓN", "parámetros del modelo de simulación"),
            dbc.Row([
                dbc.Col([
    ...
    # Fallback (tab-escenarios u otro): solo botón
    return html.Div([
...
stores = html.Div([
    ...
    # Guarda los últimos valores de los controles para que el pipeline
    # pueda leerlos aunque no estén montados en el DOM
    dcc.Store(id="store-mes-idx",     data=1),
    dcc.Store(id="store-factor-dem",  data=1.0),
    dcc.Store(id="store-cap-horno",   data=3),
    dcc.Store(id="store-opciones",    data=[]),
])
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div(id="panel-config-wrapper",
...
# ── 1. Navegación lateral → active tab ───────────────────────────────────────
...
# ── 2. Renderizar panel de configuración condicional ──────────────────────────

@app.callback(
    Output("panel-config-wrapper", "children"),
    Input("store-active-tab", "data"),
)
def render_panel_config(tab):
    tab = tab or "tab-demanda"
    return html.Div(
        build_panel_config(tab),
        style={**CARD_STYLE, "marginBottom":"0"},
    )


# ── 3. Sincronizar controles → stores (solo cuando existen en el DOM) ─────────
#    Los controles de simulación solo existen en ciertos tabs.
#    Usamos pattern-matching seguro: si el componente no existe, no dispara.

@app.callback(
    Output("store-mes-idx",    "data"),
    Output("store-factor-dem", "data"),
    Output("store-cap-horno",  "data"),
    Output("store-opciones",   "data"),
    Input("dd-mes",       "value"),
    Input("sl-demanda",   "value"),
    Input("sl-horno",     "value"),
    Input("chk-opciones", "value"),
    State("store-mes-idx",    "data"),
    State("store-factor-dem", "data"),
    State("store-cap-horno",  "data"),
    State("store-opciones",   "data"),
    prevent_initial_call=True,
)
def sync_controls(mes, factor, horno, opciones,
                  s_mes, s_factor, s_horno, s_opciones):
    """
    Actualiza los stores de parámetros solo con los valores que
    realmente cambiaron. Si un control no existe en el DOM, su
    valor llega como None → conservamos el valor anterior del store.
    """
    return (
        mes      if mes      is not None else s_mes,
        factor   if factor   is not None else s_factor,
        horno    if horno    is not None else s_horno,
        opciones if opciones is not None else s_opciones,
    )


# ── 4. Ejecutar pipeline ──────────────────────────────────────────────────────
#    Lee siempre de los stores (no de los controles directamente),
#    así funciona independientemente de qué tab está activo.
...
def ejecutar_pipeline(n_clicks, mes_idx, factor_dem, cap_horno, opciones):
    ...
    # Valores por defecto si el store aún no tiene nada
    mes_idx   = mes_idx   if mes_idx   is not None else 1
    factor_dem= factor_dem if factor_dem is not None else 1.0
    cap_horno = cap_horno  if cap_horno  is not None else 3
    ...
    try:
        ...
        import traceback
        return (None,)*7 + (f"✗ Error: {str(e)} — {traceback.format_exc()[-300:]}",)


# ── 5. Correr escenario individual ────────────────────────────────────────────
...
# ── 6. Renderizar contenido del tab activo ────────────────────────────────────
    ...
    if tab == "tab-escenarios":
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.Div(style={"height":"12px"}),
                html.Button(
                    "▶ CORRER ESCENARIOS SELECCIONADOS", id="btn-esc", n_clicks=0,
...
# ── 7. Actualizar gráfico de comparación cuando cambia el store ───────────────
